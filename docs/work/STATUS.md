...
仅在验收节点更新业务里程碑；2026-09-09文档唯一正文迁移不新增业务验收，现行维护关系见[INDEX](../INDEX.md)。前两项为2026-09-09验收；2026-09-10用户确认第三项、日志内部组件及完整控制台与文件运行诊断服务验收，包含截点报告、饱和计数及临时目录修复。表中为对应提交记录的验证依据，不代替当前工作区检查。
...
| 控制台与文件运行诊断服务：用户已验收；公开配置接入与服务、截点报告、flush／close、有界应急、恢复探测、真实文件轮转／保留，包含饱和计数及临时目录修复 | 本记录所属提交（父提交`8189d33c3104240346979d0952f83a085a8d81a9`） | 修复轮320项unittest通过（保留原313项）、Pyright 1.1.411全量零错误／警告／信息、编译及离线锁文件检查通过；验收收尾核对61份Python及3份工程文件的指纹与已测版本一致，按授权复用；显式19份文件暂存及差异检查，完整指纹与命令见CURRENT_TASK |
...
完整控制台与文件运行诊断服务已验收，验证依据及提交范围见[当前任务](CURRENT_TASK.md)。单缓冲构造依赖已核验的CPython 3.12.13 BytesIO共享路径，换解释器须重新核验。并发、截点、超时与迟到完成通过可控时钟／事件及内存故障注入验证；本机 Darwin 25.6.0 arm64 的真实自有临时文件覆盖写出、轮转、保留、重新打开、异常记录／条目和目录独占。永久阻塞资源可在close返回后继续占用线程／目录，cleanup_pending如实反映。Linux平台行为、Docker、性能目标、生产装配及G2不在本次验收范围；未做强杀／掉电或持久化验证，未查看编辑器Pylance诊断。
