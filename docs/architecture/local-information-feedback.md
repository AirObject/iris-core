...
本节保留通用目标时序及DISABLED／TEST_HTTP原行为；新增WS模式的受控路由、发送与宽限按[§6.4–6.6](#ws-reminders)实施，旧格式闭集不变。
...
外部通信补全已批准生产WS模式；其执行范围与验收见[通信实施路径](external-communication.md)。本节只扩展新通信装配，不改变DISABLED／TEST_HTTP和旧记录的解释。逻辑路由与宿主注册归[管理身份契约](managed-runtime-and-deployment.md#communication-identity)，连接消费者及按路由接管归[WS协议](external-communication.md#ws)。

目标改路由沿用`change_deadline`的原键与expected_revision（其条件字段已含route_id），同一事务取消未发旧计划，并按新路由生成计划。route属于目标精确合并的结构签名，改路由后与其他目标的合并关系按新结构判定；保留§6.2仅对新目标启动一次精确去重的规则，改路由更新结构签名和后续候选核验，不另建重复去重任务；未决语义候选继续重查双方revision。不得拆分已有别名、丢失来源，或把不同路由的目标静默合并。

路由权限要贯通三处：宿主目标写、管理员目标写、内部目标／提醒调度。内部目标使用真实可信工作身份及来源权限，不能改为管理员任意构造目标。调度获取当前受控路由能力，不能只复用测试接收器绑定或伪造`route_ids`。后续撤销／禁用采用revision检查，已发送事实不回滚；新派发末端重查当前权限和目标状态。

无在线消费者或路由禁用不影响合法有deadline目标的保存；未知或越权路由拒绝。默认路由只按管理所有者提供的唯一合法候选解析，并把实际route写入目标。旧发送事实不得因改路由、启用或轮换令牌被撤销或改成待发。

<a id="ws-delivery"></a>

### 6.5 WS有限发送与断线裁决（已批准）

WS发送至ACK单次时限、登记确认至终态提交总时限、有限重试和并发上限按[通信配置](configuration.md#external-communication-configuration)固定；总时限包含末端检查等待、排队、发送、ACK等待及终态持久提交，不给子步骤重新分配完整预算。最多两次只适用于终结前可证明未发送的有限底层尝试；NOT_SENT一旦终结不重新开启，订阅接管不授予换连接重试权。ACK未知、迟到与终态约束见[消息确认协议](external-communication.md#ws)。

沿用本节既有提醒与未知结果规则：先持久登记尝试并确认，再在短串行区检查模式、目标revision、路由权限及连接所有权，事务外开始发送。WS排队仅表示本地持有，不是远端已接收；实际调用发送、可能写入网络的边界必须保守标记。

到期计划按路由轮转选取。只为当前有合格在线消费者且有尝试名额的计划登记尝试；合格指路由启用且revision一致、连接已认证并订阅该事件、队列有容量、连接未排空且该路由订阅不在接管中。每路由同时至多一个在途尝试，全局在途数有界。等待消费者或尝试名额的计划按[宽限计时与恢复](#ws-grace)保存宽限，不占尝试名额，也不阻塞其他路由；不得因每轮重扫或重连重新计时。已登记的尝试不再回到宽限。

| 情况 | 推荐处置 |
| --- | --- |
| 计划到期，sink为DISABLED或路由已禁用 | 持久终结为未发送／接收端不可用，保留目标 |
| 计划到期，路由启用但无合格在线消费者 | 宽限期内保持待发，不阻塞其他路由；期间消费者上线则正常登记并发送；宽限结束终结为未发送，之后上线不补发 |
| 已登记，可证明未开始网络发送时连接失效或本路由订阅被接管 | 从本路由队列原子撤回，按NOT_SENT终结，不转给新连接，也不重新进入宽限；本次终结后不再尝试 |
| 已开始发送，收到匹配且经授权的ACK | 原尝试提交ACKNOWLEDGED；ACK重复只确认同一事实 |
| 已可能发送但无ACK、本尝试所属路由的订阅被接管、进程中断或结果不明 | UNKNOWN，恢复只本地核对；不得更换连接／delivery标识再发；其他路由的接管不影响本尝试 |
| 取消／禁用先越过末端检查 | 零新发送，保留取消／未发送原因 |
| 发送先开始后发生取消／撤销 | 保留真实发送及清理责任；不承诺撤回在途字节，不再向撤销身份交付新内容 |

目标仍由goals唯一记录计划及attempt。探针需最小独立事件与尝试记录、有限保留及汇总，由management明确所有权；模式提示不建记录。两者都不复用Provider账本，不建立可重放日志总线。日志保留策略不可删除仍未确认或仍持有资源的记录；历史清理范围先在容量步骤核算。

原有DISABLED已终结计划和旧UNKNOWN不能因启用WS而重新变成待发送。重连后宿主通过现有目标列表／受控运行查询同步当前状态。要求离线必达或历史订阅游标时，应另行批准投递保障及容量，本方案不暗中实现。

<a id="ws-grace"></a>

### 6.6 宽限计时、专注与持久恢复（已批准）

宽限只适用于WS模式下从未登记尝试、仍有效的待发计划。按计划冻结的配置取得时长G（取[通信配置](configuration.md#external-communication-configuration)的已批准小档值），goals以稳定计划键幂等保存起算时间、时长／配置引用、绝对截止时间及必要的暂停／恢复事实；字段和完整编码预算在[实施步骤0](external-communication.md#implementation)冻结。初始化或变更结果待确认时沿原键及原输入续办，不能重新选时间或先派发。计时状态归goals，不以连接内存或临时扫描游标为准。

| 条件／转换 | 计时和计划处理 |
| --- | --- |
| 首次具备领域派发条件 | 计划已到期、目标仍有效、去重等待已按原规则解除，且专注／维护派发门控开放；以首次受控推进的服务端时间t0持久初始化截止t0＋G，确认后生效。是否已有消费者、队列或尝试名额不影响起算；已有消费者且有名额可在同次推进中登记尝试 |
| 尚未具备领域派发条件 | 未到期、去重等待或门控关闭期间不启动宽限。创建已过期目标、截止改到过去或专注跨期后，从首次具备条件的推进时刻起算，不能机械用历史due_at＋G使其立即失效 |
| 宽限开始后专注／维护暂停 | 在可信门控关闭截点保存尚余时长，暂停期间不消耗；多个关闭原因重叠只暂停一次，全部解除后才恢复。恢复时以原剩余时长计算新的绝对截止，不能重新给满G；关闭前已过期的计划先按过期处置，不得借暂停复活 |
| 客户端离线、排队或无尝试名额 | 继续消耗宽限；心跳、订阅接管、重连、反复扫描及配置激活／回退均不重置既有计划的计时和冻结时长 |
| 进程意外中断／重启 | 已起算且未暂停的计划沿用持久绝对截止，停机时间计入宽限；已确认暂停的计划按原暂停事实恢复剩余时间。恢复本身不产生一次新的暂停或新窗口；尚未起算的计划仍按首次领域可派发条件处理 |
| UPCOMING跨过目标deadline | 无论未起算、计时中还是暂停，均按原覆盖规则标SUPERSEDED；恢复后的截止不得越过目标deadline。DUE是独立计划，仅在自身未登记尝试且未终结时按本表推进；不顺延目标deadline |
| 未暂停的窗口截止已到且仍未登记尝试 | 先核对暂停／恢复事实，再以now≥有效绝对截止判过期，持久终结为未发送；即使本轮消费者刚上线，也先处理过期。暂停中不依据暂停前的旧截止误判过期。登记与过期／暂停裁决串行重查计划revision，避免边界上同时发送和终结 |

专注／维护暂停依据必须来自门控所有者可持久确认、可原键恢复的转换事实，不能用调度器下一次扫描时间猜测暂停起点。恢复先核对门控事实再推进计划；交接未知时关闭受影响的新派发并续办。门控所有者按[计时交接契约](../modules/runtime.md#communication-gate-clock)提供依据，实施步骤0须冻结门控与goals之间容量有界的交接／恢复协议，覆盖转换前后中断及重复确认，不另建可重放事件总线。路由禁用、sink变为DISABLED、目标取消或完成仍按各自终结规则处理，不作为可以保留窗口的暂停。

例如，DUE宽限已消耗100秒后进入专注，恢复后只剩200秒；未暂停时进程在相同截点中断50秒，恢复后只剩150秒。专注期间才到期且从未起算的DUE，在恢复并解除去重等待后才启动一次窗口；已登记或已终结的计划均不适用这些例子。
