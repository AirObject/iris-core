...
本节是旧Information装配的[集中契约](local-information-feedback.md)配置唯一补充；托管WS增量独立见[§11.17](#external-communication-configuration)。**新增定义、数值、完整组合及持久层工程增量已随五组推荐获用户批准**；旧§11.15及其他已批准定义不变。已授权注册参数、实现配置与装配及范围内验证；不批准未采纳替代、热改或存量迁移。推荐新独立`resolve_information_configuration`、对应persist/load及只读适用性端口，输入为完整foundation/runtime/platforms/content/information、受保护目录及材料声明；旧resolve_content_configuration继续拒绝新域，不能删除新键或读环境补齐。
...
<a id="external-communication-configuration"></a>

### 11.17 托管外部通信配置（已批准）

本节是[外部通信补全](external-communication.md)的配置唯一正文。新增WS生产sink及管理／传输参数只进入明确版本的新通信装配；§11.16原Information格式仍只接受DISABLED／TEST_HTTP及原数值，不修改旧解析器、固定向量或持久字节。托管版本选择WS时使用本节时序／并发，选择DISABLED／TEST_HTTP时维持各自原语义。新安装和升级默认不开放WS，启用必须经过显式配置激活。

#### 11.17.1 数据来源与生效归属

| 数据类别 | 唯一来源 | Web可做的事 |
| --- | --- | --- |
| 数据目录、秘密根、实际监听绑定／端口、外部origin、可信代理来源、进程数、代理证书挂载 | 部署参数及受保护资源，受控重启生效 | 查看脱敏状态、实际请求Host／Origin是否匹配、受控重启说明，生成不含秘密的代理配置片段；不能修改这些值、上传私钥、修改任意文件或执行部署命令 |
| WS策略、连接限制、提醒时序／并发／宽限、接管与探针限额 | 统一配置注册表与持久版本 | 编辑、验证、预览、激活／回退；标明需重连／重启及实际应用情况 |
| 宿主／入口绑定、逻辑路由、令牌权限及撤销 | ingress／management管理实体 | 通过各自原键与revision端口操作，分别产生必要审计 |
| 在线连接、队列深度、最后心跳 | 通信运行时受限投影 | 查看／请求断开，不把进程内状态备份成可恢复连接 |
| 目标及投递事实 | goals；其他通知由明确的运行／管理owner负责 | 只读观察或原领域操作，不能直接编辑投递终态 |

当前`IRIS_DEPLOYMENT`仍负责引导所需的真实启动资源。新增可在线配置的通信策略不能另开环境变量覆盖通道：服务器从配置模块取得已确认有效版本；未初始化时按最小引导策略运行，宿主WS关闭。新增持久配置如何在业务宿主恢复前被可信读取，需要在装配步骤验证；配置源不一致或恢复失败时关闭受影响准入，保留管理恢复入口，不能回退到宽松默认。

外部origin、监听与可信代理来源的校验只在[管理部署身份](managed-runtime-and-deployment.md#external-origin)维护；通信参数的消费者接入、排空、撤销及回退只在[管理激活协议](managed-runtime-and-deployment.md#configuration)维护，不能用环境变量覆盖已确认运行版本。

#### 11.17.2 完整小档、Schema与容量

以下为已批准的首个完整小档向量，尚不代表压测结果。具体键名、封闭Schema及该小档预算内的合法组合由执行者按统一注册表实物核算冻结，旧格式值与旧冻结工作不得覆写。合法范围、依赖、敏感级别、读写角色、apply_mode和完整编码预算都必须进入Schema；只有经过组合核算的值可声明支持。

| 参数组 | 已批准小档值 | 生效与约束 |
| --- | --- | --- |
| WS开关 | 新安装及升级默认关闭 | 显式激活后接受新连接；关闭先停止新派发，原在途按原事实收尾 |
| WS连接 | 全局16；每路由1；未认证并发2；未认证期限5秒 | 连接准入时生效；下调不任意取消已开始业务，预览关闭／排空方式 |
| 订阅与路由 | 每连接至多8条授权订阅；路由总量先按16核算 | 不突破现有身份／入口承载；完整记录及ACL编码一起验证 |
| 消息及队列 | 业务通知完整编码≤2048字节，ACK≤1024；每连接发送队列16条且总字节≤32768 | 包含封套；临时缓冲、分片重组、框架队列及多份编码副本计入总内存 |
| 心跳 | Ping间隔20秒，Pong等待10秒 | 新连接使用新值；代理空闲时限须显式大于心跳周期和余量 |
| 提醒时序（WS模式） | 发送开始到ACK单次5000ms；登记确认到终态提交总计12000ms；最多2次，且仅已证明未发送可再尝试 | 只用于WS模式，DISABLED／TEST_HTTP沿用原值；超时记UNKNOWN，不另加无界等待；关闭排空与代理空闲时限须覆盖总时限 |
| 提醒并发与宽限 | 全局在途尝试4、每路由1；按路由轮转，每轮至多扫描16条到期计划；未发送宽限300秒 | 起算、暂停、持久截止与恢复统一按[宽限计时与恢复](local-information-feedback.md#ws-grace)；待发计划不占尝试名额；多路由同时到期的完成时延须实测 |
| 接管 | 每路由每10秒至多1次 | 超限拒绝，并在连接页提示可能的实例争用 |
| 重连建议 | 初始1秒、指数退避、上限30秒、随机扰动 | 仅连接恢复，不能驱动通知或写请求自动重放 |
| 测试票据／探针 | 票据30秒、一次消费；全局每分钟至多3次新探针（当前仅单个管理员） | 绑定会话／路由／用途；限额计数归受控管理，不能绕过专注或令牌撤销 |
| HTTP与媒体 | 复用现有JSON命令和媒体上限，流式块单独受限 | 不扩大普通命令、配置初始化或blob预算，禁止累积完整大请求后才检查 |

该档通知队列仅原始字节预算为16×32768＝524288字节；这不含服务器接收队列、TLS／socket、对象、任务和持久资源，不是进程总内存保证。实施必须给出完整资源计算和实物编码，覆盖连接全占用、慢接收、认证失败、上传／备份同时存在以及路由／令牌最大组合；不能用压缩或截断合法对象凑限额。

新增定义必须包含owner、消费者、单位、上下界／枚举、依赖、敏感级别、读写角色、apply_mode、错误闭集和升级解释；界面只能编辑已声明合法的组合。按新连接／新计划／新尝试分别绑定版本，不给已冻结宽限替换时长。传输压缩首版关闭；连接数、订阅、消息、分片重组、队列和未完成I/O均须核算实际占用。

统一托管配置、普通命令、唯一配置初始化及静态装配仍受[现有总预算](managed-runtime-and-deployment.md#upgrade)约束。新增记录／参数／门控交接做完整实物编码，不能删审计、截断合法对象、隐藏定义或增加环境变量旁路来适配。首档内具体工程数值／类型选择已授权；不能容纳或需要扩大资源档时仅停止受影响部分并提交证据。启用现有受控升级路径不意味着任意旧库迁移。
