...
2026-09-14按实际能力重排为四项完整交付，替换原按模块递进的阶段表。历史顺序可通过Git查阅；本表只维护依赖顺序与范围，各项已交付状态、实际提交及剩余验证统一见[STATUS](../work/STATUS.md)，不另维护阶段计数。本表不自动批准尚未定稿的产品细化、实现、调用或资源。已实现的持久接入、三段运行、正式记忆／来源／媒体文件、本地查询／反馈／状态／目标，以及有限文本学习和首次persona均复用，实际版本和限制见[STATUS](../work/STATUS.md)。
...
| 4．可部署、可管理的首期产品 | [集中计划与已批准决定](managed-runtime-and-deployment.md)：管理Web、统一宿主、初始化与时区、身份／凭据、配置激活／回退、日志、备份恢复、升级及Docker交付；以前项修复、验收和提交为前置 | 新环境可安装、初始化、接入宿主、持续管理并恢复；只对实际验证的部署和负载声明支持 |
...
| 初始化、配置和管理权限 | [已批准集中计划](managed-runtime-and-deployment.md)、[管理行为](../product/operations-and-management.md)：同一完整交付，当前实施前置见CURRENT_TASK |
