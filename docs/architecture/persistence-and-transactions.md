...
**状态：本节新装配、结果绑定审计及兼容方案已获用户批准；生产方案与实际验证边界见§10.4。** 属于[整体契约](durable-ingress-and-batch-runtime.md)；§4、§7、已批准§8／§9的确认、唯一提交、资源占用、格式拒绝及审计保证不变。仅在自有测试资源执行验证，不实现迁移；执行授权及证据见CURRENT_TASK。整体状态、行为和验收在主契约；本节只维护新装配、类型化事务参与、命令／回执兼容和生产资源边界；事务派生审计所需的精确增量见下文，未将其写入已批准§8／§9。
...
首期生产方案已获用户批准，唯一行为见[部署与管理集中计划](managed-runtime-and-deployment.md)。原持久接入组合的临时资源验收不因此扩为生产资格；早期反向代理身份服务与试验库迁移建议未被选用，也不保留为并行实施方案。

| 前置 | 唯一维护位置与边界 |
| --- | --- |
| 生产路径与G2 | [生产目录](managed-runtime-and-deployment.md#storage)及[日志完整资源清单](logging.md#runtime-diagnostics-directory-sources)；非public路径能力与真实权限／别名／占用须一同验证 |
| 可信建库身份 | [装配与持久启动身份](managed-runtime-and-deployment.md#assembly)；CREATE_NEW之前持久保留外部期望，不从目标库反推 |
| 管理与宿主鉴权 | [本地管理员及受限宿主令牌](managed-runtime-and-deployment.md#identity)；回环、测试身份或请求自报角色都不能代替认证 |
| 实例、备份与升级 | [一致备份恢复](managed-runtime-and-deployment.md#storage)、[兼容版本升级](managed-runtime-and-deployment.md#upgrade)；新实例先行，历史试验库不自动迁移，不能通过旧备份丢弃新数据 |
