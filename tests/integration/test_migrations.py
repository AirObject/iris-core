...
def test_repository_migrations_apply_on_empty_database(tmp_path: Path) -> None:
    ...
    assert [item.version for item in applied] == [1, 2, 3, 4, 5, 6]
    assert runner.migrate() == ()
...
def test_phase0_database_upgrades_to_phase1(tmp_path: Path) -> None:
    ...
    assert [item.version for item in upgraded] == [2, 3, 4, 5, 6]
    assert MigrationRunner(database, default_migrations_path()).current_version() == 6
