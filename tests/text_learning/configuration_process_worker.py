...
async def main(root: Path, mode: str, barrier: str) -> None:
    ...
    assembly = TextConfigurationAssembly()
    storage = PersistenceService(assembly.repositories, assembly.commands, assembly_format='MODEL_TEXT_LEARNING_V1')
    ...
    try:
        result = await binding.persist_text_learning_configuration('original-config', value, actor='bootstrap', protected_directories=supplied[6])
