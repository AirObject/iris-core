...
from companion_memory.configuration import (
    ...
    NotApplicable,
    ParameterDefinition,
    ParameterDefinitionInput, MetadataValue, FrozenMetadataValue,
    RangeDescriptor,
    Unbounded,
...
class RegistryLifecycleTests(RegistryTestCase):
        ...
        for name, value in submitted.items():
            with self.subTest(field=name):
                if type(value) is list:
                    ...
                    value = Declared(tuple(self.declared(submitted["enum"])))
    ...
    def test_empty_freeze_is_idempotent_and_prevents_all_registration(self):
        ...
        # Frozen-state precedence is checked even for malformed definitions.
        for submitted in (None, {}, definition(), {"key": " "}):
            with self.subTest(input_type=type(submitted).__name__):
                self.failure(builder.register(cast(ParameterDefinitionInput, submitted)), "REGISTRY_FROZEN", "register",
                             [((), "REGISTRY_FROZEN")])
        self.assertEqual(registry.list_definitions(), ())
    ...
    def test_query_checks_key_before_existence_without_fallback(self):
        ...
        # Invalid key carriers must reach validation unchanged.
        for key in (None, 0, False, [], {}, "", " key", "a\tb", "a\u2003b"):
            self.failure(registry.get_definition(cast(str, key)), "INVALID_PARAMETER_KEY",
                         "get_definition", [(("key",), "INVALID_IDENTIFIER")])
        self.failure(registry.get_definition("absent"), "UNKNOWN_PARAMETER",
                     "get_definition", [(("key",), "UNKNOWN_KEY")])
        with self.assertRaises(TypeError):
            # This unsupported keyword is itself the subject of the runtime test.
            registry.get_definition("absent", default=None)  # pyright: ignore[reportCallIssue]

    def test_duplicate_registration_preserves_first_definition(self):
        builder = create_registry_builder()
        original = definition()
        self.success(builder.register(original))
        # Duplicate-key precedence must also hold for an incomplete declaration.
        for submitted in (definition(), definition(owner_module="another_owner"),
            ...
            self.failure(builder.register(cast(ParameterDefinitionInput, submitted)), "DUPLICATE_PARAMETER", "register",
                         [(("key",), "DUPLICATE_KEY")])
        registry = self.success(builder.freeze())
    ...
    def test_invalid_registration_leaves_key_available_and_input_untouched(self):
        ...
        members = self.declared(submitted["enum"])
        self.assertIs(type(members), list)
        original_members = cast(list[MetadataValue], members).copy()
        ...
        self.assertEqual(self.declared(submitted["enum"]), original_members)
...
class RegistryOwnershipTests(RegistryTestCase):
    def test_mutating_inputs_before_freeze_cannot_change_registered_metadata(self):
        label: dict[str, MetadataValue] = {"label": "kept"}
        shared: dict[str, list[MetadataValue]] = {"items": [None, False, 0, Decimal("1.20"), label]}
        members = [shared]
        ...
        for name in ("scope", "read_roles", "write_roles", "validator", "consumers",
                     "dependencies"):
            # Each named field in this fixture contains the original mutable list.
            cast(list[str], cast(dict[str, object], submitted)[name]).append("changed")
        submitted["key"] = "changed"
        cast(dict[str, object], submitted).clear()
        registry = self.success(builder.freeze())
        stored = self.success(registry.get_definition("sample.label"))
        expected = (None, False, 0, Decimal("1.20"), {"label": "kept"})
        self.assertEqual(self.mapping(self.defaulted(stored.default))["items"], expected)
        self.assertEqual(self.mapping(self.declared(stored.enum)[0])["items"], expected)
    ...
    def test_returned_records_and_every_nested_container_reject_mutation(self):
        ...
        for stored in (queried, enumerated):
            ...
            members = self.declared(stored.enum)
            first_member = self.sequence(members[0])
            default_item = self.mapping(default[0])
            ...
            for sequence in (stored.scope, stored.validator, stored.dependencies,
                             ...
                             members, first_member, default,
                             self.sequence(default_item["items"])):
                self.assertIsInstance(sequence, tuple)
                with self.assertRaises((AttributeError, TypeError)):
                    # Attempt mutation on the tuple itself; the cast performs no conversion.
                    cast(list[object], sequence).append("changed")
            ...
            for mapping in (default_item, self.mapping(first_member[0])):
                with self.assertRaises(TypeError):
                    cast(dict[str, FrozenMetadataValue], mapping)["items"] = "changed"
            self.assertFalse(hasattr(stored, "__dict__"))
        with self.assertRaises((AttributeError, TypeError)):
            cast(list[ParameterDefinition], registry.list_definitions()).append(queried)
        self.assertEqual(self.mapping(self.sequence(self.defaulted(queried.default))[0])["items"], (None, 0))
    ...
    def test_range_and_explicit_markers_are_immutable(self):
        ...
        limits = self.declared(stored.range)
        for item, name in ((stored.range, "value"), (limits, "lower"),
                           (limits.lower, "inclusive"),
                           (limits.lower, "value"),
                           (limits.upper, "extra"), (NoDefault(), "extra")):
            with self.assertRaises((AttributeError, TypeError)):
                setattr(item, name, None)

    def test_errors_and_results_are_immutable_and_only_contain_safe_fields(self):
        builder = create_registry_builder()
        # Keep malformed and sensitive fields intact at the public input boundary.
        result = builder.register(cast(ParameterDefinitionInput,
                                       {"key": "sensitive key", "credential": "hidden"}))
