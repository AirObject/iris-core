...
class MaintenanceAssembly:
    ...
    def handle(self, kind: str, uow: UnitOfWork, v: MappingProxyType[str, Value], *, expiry=None, administration=None):
        ...
        if administration is not None:
            from .administration import MemoryAdministration
            if type(administration) is not MemoryAdministration or administration.content is not content or expiry is not None:raise InvalidValue()
            administration.verify_active(uow)
        authority = expiry if expiry is not None else administration
        if authority is None and mode['state'] not in ('NORMAL', 'DRAINING'):
        ...
        if kind == 'plan_memory_change':
            ...
            if roots:
                ...
                definition = (authority.definition(cast(str, previous['command_kind'])) if authority is not None else self.content.command_definition(cast(str, previous['command_kind'])))
            ...
            command_kind = (administration.command_prefix if administration is not None else 'expire_dream_memory_' if expiry is not None else 'apply_memory_') + mask.lower()
            checksums = tuple(digest(leaf, 8192) for leaf in leaves)
            pid = stable('release_plan', root_id, ordinal, semantic, checksums)
            execution_key = stable('memory_execution', root_id, ordinal, pid) if authority is None else cast(str,v['operation_id'])
            ...
            if authority is not None:return plan
        ...
        if len(memory.rows.stage('root_success', uow, {'root_id': plan['root_id'], 'plan_id': plan['plan_id'], 'ordinal': plan['ordinal']})) != 1:
            raise OwnerFailure('PRECONDITION_FAILED', 'source', 'OWNERSHIP_CHANGED')
        if authority is not None:return applied,release
