...
SEMANTIC_TICKET=RecordSchema(tuple(Field('version',ScalarSchema('integer',1,2)) if field.name=='version' else field for field in TICKET.fields))
...
SEMANTIC_DISPOSITION=RecordSchema(tuple(Field('version',ScalarSchema('integer',1,2)) if field.name=='version' else field for field in DISPOSITION.fields)+(Field('response_digest',TEXT(64)),))
