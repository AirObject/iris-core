...
class SemanticQuery:
    ...
    async def select_readonly(self,memory,memory_port,index,partition:str,query_text:str,world:Record,resources:AsyncExitStack,deadline:float):
        ...
        if (type(memory) is not MemoryService or type(memory_port) is not MemoryReadPort or type(index) is not LocalIndex
                or (type(index.configuration) is not StoredDailyConfiguration and type(index.configuration) is not StoredDreamConfiguration and type(index.configuration) is not StoredManagedConfiguration) or index.configuration is not self.cache.configuration
