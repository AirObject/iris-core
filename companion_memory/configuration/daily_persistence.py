...
from .daily_resolution import DailyMaterialContract as MaterialContract
...
from .daily_codec import candidate_values as _candidate_values
from .daily_resolution import (
    DailyConfigurationCandidate,DailyConfigurationErr,DailyConfigurationOk,
    configuration_failure,resolve_daily_configuration,daily_snapshot_issue,
)
from .daily_persistent_results import ConfigurationCommitted,ConfigurationNotCommitted,ConfigurationRejected,ConfigurationUnconfirmed
...
def stored_daily_configuration_issue(value: object) -> str | None:
    """Check native durable issuance and the complete reconstructed candidate."""
    if type(value) is not StoredDailyConfiguration or getattr(value, '_issuer', None) is not _STORED_ISSUER:
    ...
    try:
        ...
        return 'DEFINITION_MISMATCH' if daily_snapshot_issue(value.candidate) is not None else None
...
class DailyConfigurationAssembly:
    ...
    def __init__(self,memory_catalog:StatementCatalog|None=None,retrieval_catalog:StatementCatalog|None=None):
        ...
        self.repository=replace(repository, definition=replace(repository.definition, schema_version=6))
        self._binding: DailyConfigurationBinding | None = None
        requirement=AuditRequirement('configuration','configuration_initialized','CONFIGURATION_INITIALIZED',1,('INITIALIZE',),CHANGE)
        definition=ResultBoundCommandDefinition('configuration','initialize_daily_configuration',1,
        ...
        from companion_memory.persistence.command_capacity import _issue_daily_configuration_capacity
        definition=replace(definition,capacity_policy=_issue_daily_configuration_capacity(self))
    ...
    def bind(self,storage:PersistenceService,instance_id:str,bootstrap:DailyConfigurationCandidate) -> DailyConfigurationBinding:
        if self._binding is not None:
            raise ValueError('Configuration assembly is already bound.')
        if daily_snapshot_issue(bootstrap) is not None:
            raise ValueError('A complete native daily configuration is required.')
        binding=DailyConfigurationBinding(self,storage,instance_id,bootstrap)
        self._binding=binding
        return binding


type ConfigurationWriteResult = ConfigurationCommitted | ConfigurationNotCommitted | ConfigurationRejected | ConfigurationUnconfirmed | DailyConfigurationErr
type ConfigurationReadResult = DailyConfigurationOk[StoredDailyConfiguration] | DailyConfigurationErr


class DailyConfigurationBinding:
    """Restricted initialization and point loads through configuration-owned ports."""
    def __init__(self,assembly:DailyConfigurationAssembly,storage:PersistenceService,instance_id:str,bootstrap:DailyConfigurationCandidate):
    ...
    async def initialize_business_roots(self,key:str,stored:StoredDailyConfiguration):
        ...
        """
        if self._closed or self._root_operation is None or stored_daily_configuration_issue(stored) is not None or self._tasks:
            raise InvalidValue()
    ...
    def participate_snapshot(self, uow: UnitOfWork, expected: StoredDailyConfiguration) -> bool:
        """Verify the active durable snapshot inside another owner's transaction."""
        if (self._closed or stored_daily_configuration_issue(expected) is not None
    ...
    async def persist_daily_configuration(self,operation_key:object,candidate:object,*,actor:str,protected_directories:object) -> ConfigurationWriteResult:
        """Bound the complete publication/load while retaining an unfinished owner."""
        operation='persist_daily_configuration'
        if self._closed or type(candidate) is not DailyConfigurationCandidate or daily_snapshot_issue(candidate) is not None:return configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH',operation)
        from .daily_codec import candidate_inputs
        preflight = resolve_daily_configuration(*candidate_inputs(candidate, protected_directories))
        if type(preflight) is DailyConfigurationErr:
            return DailyConfigurationErr(replace(preflight.error, operation=operation))
    ...
    async def _publish(self, operation_key: object, command: ResultBoundCommand, handle: RecoveryHandle,
                       candidate: DailyConfigurationCandidate, protected_directories: object) -> ConfigurationWriteResult:
        with CompletionScope() as completion:
            ...
            if type(result) is not MappingProxyType:
                failure = configuration_failure('INTEGRITY_FAILURE', 'storage', 'CONTENT_MISMATCH', 'persist_daily_configuration')
            ...
            if isinstance(loaded, DailyConfigurationOk):
    ...
    async def active_snapshot_id(self) -> object:
        ...
        if self._closed:
            return configuration_failure('ACCESS_DENIED', 'identity', 'BINDING_MISMATCH', 'load_daily_configuration')
    ...
    async def _read_active_snapshot_id(self) -> object:
        ...
        if type(result) is Found and type(result.value) is tuple:
            ...
            if type(row) is not MappingProxyType:
                return configuration_failure('INTEGRITY_FAILURE', 'storage', 'CONTENT_MISMATCH', 'load_daily_configuration')
            return Found(row['snapshot_id'])
        return result

    async def load_daily_configuration(self,snapshot_id:object,material_contracts:tuple[MaterialContract,...],protected_directories:object,*,bootstrap:DailyConfigurationCandidate) -> ConfigurationReadResult:
        """Bounded whole-domain reconstruction; timeout cannot release a live reader."""
        if self._closed or daily_snapshot_issue(bootstrap) is not None:return configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','load_daily_configuration')
        if self._tasks:return configuration_failure('PERSISTENCE_FAILED','storage','READ_FAILED','load_daily_configuration')
        ...
        return logical.result() if done else configuration_failure('PERSISTENCE_FAILED','storage','READ_FAILED','load_daily_configuration')

    async def _load(self,snapshot_id:object,material_contracts:tuple[MaterialContract,...],protected_directories:object,*,bootstrap:DailyConfigurationCandidate) -> DailyConfigurationOk[StoredDailyConfiguration] | DailyConfigurationErr:
        """Reconstruct complete definitions and values, refusing missing or altered rows."""
        operation='load_daily_configuration'
        ...
        try:
            ...
            if set(catalog)!= {'version','domains','materials'} or type(catalog['version']) is not int or catalog['version']!=6:
            ...
            resolved=resolve_daily_configuration(domains['foundation'],domains['runtime'],platforms,domains['content'],domains['information'],domains['daily_cognition'],protected_directories,material_contracts)
            if type(resolved) is not DailyConfigurationOk:
            ...
            stored=object.__new__(StoredDailyConfiguration)
            ...
            return DailyConfigurationOk(stored)
        except OSError:
...
def stored_configuration_issue(value:object) -> bool:
    ...
    if type(value) is not StoredDailyConfiguration:return True
    try:return object.__getattribute__(value,'_issuer') is not _STORED_ISSUER or daily_snapshot_issue(value.candidate) is not None
