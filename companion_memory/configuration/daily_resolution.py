"""Independent complete daily configuration resolution and native applicability.
...
from .daily_schema import DAILY_RUNTIME_REQUIREMENTS, DAILY_CONTENT_REQUIREMENTS, material_definitions, matches_material_definition
from .daily_validation import validate_daily_values, validate_inherited_vector, validate_relationships, DailyValueError
from .daily_schema import DAILY_PROVIDER_REQUIREMENTS
...
@dataclass(frozen=True,slots=True)
class DailyConfigurationErr:
    error: DailyConfigurationError


@dataclass(frozen=True,slots=True)
class DailyConfigurationOk[T]:
    value: T


def configuration_failure(code: str, field: str, reason: str, operation: str='resolve_daily_configuration') -> DailyConfigurationErr:
    return DailyConfigurationErr(DailyConfigurationError(code,operation,field,reason))


@dataclass(frozen=True,slots=True,init=False)
class DailyMaterialContract:
...
def bind_daily_material(platform_id: str) -> DailyMaterialContract:
    if not valid_identifier(platform_id):raise InvalidValue()
    result=object.__new__(DailyMaterialContract)
    for name,value in dict(platform_id=platform_id,format_version='DAILY_CONTEXT_V1',media_processing=True).items():
...
@dataclass(frozen=True,slots=True,init=False)
class DailyConfigurationCandidate:
    ...
    material_contracts: tuple[DailyMaterialContract,...]
    _issuer: object
...
def _foundation_values(snapshot,protected_directories):
    ...
    for e in entries:
        ...
        for v in e.definition.validator:
            if v.startswith(('text_','semantic_','daily_')):continue
...
def resolve_daily_configuration(foundation:object,runtime:object,platforms:object,content:object,
        information:object,text:object,protected_directories:object,material_contracts:object) -> DailyConfigurationOk[DailyConfigurationCandidate] | DailyConfigurationErr:
    ...
    if type(contract) is not DailyMaterialContract or contract!=bind_daily_material(identity):return fail('DEFINITION_MISMATCH','material','METADATA_MISMATCH')
    ...
    if len(all_states)!=130 or any(type(all_states.get(dep)) is not PresentValue for view in built.values() for e in view.list_entries() for dep in e.definition.dependencies):
    ...
    candidate=object.__new__(DailyConfigurationCandidate)
    ...
    from .daily_codec import ConfigurationCapacityExceeded
    try:
    ...
    return DailyConfigurationOk(candidate)


def daily_snapshot_issue(candidate:object) -> DailyConfigurationError | None:
    """Revalidate native ownership, metadata, values and all format relationships."""
    failure=configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','daily_snapshot_issue').error
    if type(candidate) is not DailyConfigurationCandidate:return failure
    try:
        ...
        result=resolve_daily_configuration(*candidate_inputs(candidate,directories))
        if type(result) is not DailyConfigurationOk:return failure
        from .daily_codec import candidate_values
...
def freeze_daily_domains(domains):
    """Validate all cross-domain dependencies before issuing the six projections.

    Registration remains the sole metadata validator. A dependency refers to the
    complete immutable registry, even when its storage domain differs. Duplicate
    keys, unresolved references and self-dependencies reject the whole set.
    """
    from .registry import create_registry_builder
    from .results import Ok
    if type(domains) is not dict or len(domains)!=6:raise InvalidValue()
    builder=create_registry_builder();keys={}
    for name,definitions in domains.items():
        if type(name) is not str or type(definitions) not in (list,tuple):raise InvalidValue()
        keys[name]=[]
        for definition in definitions:
            if type(definition) is not dict or definition['key'] in definition['dependencies']:raise InvalidValue()
            if type(builder.register(cast(ParameterDefinitionInput,definition))) is not Ok:raise InvalidValue()
            keys[name].append(definition['key'])
    complete=builder.freeze()
    if type(complete) is not Ok:raise InvalidValue()
    definitions={d.key:d for d in complete.value.list_definitions()}
    return {name:ReadOnlyRegistry._from_definitions({key:definitions[key] for key in names}) for name,names in keys.items()}
