...
from .managed_schema import material_definitions, matches_material_definition
from .managed_validation import validate_managed_values, DailyValueError
from .daily_validation import validate_relationships
...
_ISSUER=object()
PROTECTED_KEYS=frozenset(('storage.database_file','logging.file_directory',
    'media.root_directory','media.staging_directory'))


def _managed_common(definition):
    """Accept protected path declarations only in this explicit managed format."""
    if definition.key not in PROTECTED_KEYS:
        return _unsupported_common_declaration(definition)
    if (definition.sensitivity!='administrator' or definition.scope!=('instance',)
            or definition.override_policy!='no_override' or definition.deprecated
            or type(definition.replacement) is not NotApplicable or type(definition.upgrade_rule) is not NotApplicable):
        return 'protected_path_metadata'
    return None


@dataclass(frozen=True,slots=True)
class ManagedConfigurationError:
    code: str
    operation: str
    field: str
    reason: str
...
@dataclass(frozen=True,slots=True)
class ManagedConfigurationErr:
    error: ManagedConfigurationError


@dataclass(frozen=True,slots=True)
class ManagedConfigurationOk[T]:
    value: T


def configuration_failure(code: str, field: str, reason: str, operation: str='resolve_managed_configuration') -> ManagedConfigurationErr:
    return ManagedConfigurationErr(ManagedConfigurationError(code,operation,field,reason))


@dataclass(frozen=True,slots=True,init=False)
class ManagedMaterialContract:
...
def bind_managed_material(platform_id: str) -> ManagedMaterialContract:
    if not valid_identifier(platform_id):raise InvalidValue()
    result=object.__new__(ManagedMaterialContract)
    for name,value in dict(platform_id=platform_id,format_version='MANAGED_DAILY_CONTEXT_V1',media_processing=True).items():
...
@dataclass(frozen=True,slots=True,init=False)
class ManagedConfigurationCandidate:
    ...
    material_contracts: tuple[ManagedMaterialContract,...]
    _issuer: object
...
def _foundation_definitions(definitions) -> bool:
    ...
    for d in definitions:
        if _managed_common(d) is not None or any(bindings.get(v)!=(d.key,d.type) for v in d.validator):return False
    for r in DAILY_PROVIDER_REQUIREMENTS:
        d=by_key.get(r.key)
        if d is None:return False
        comparison=d
        if r.key=='provider.profiles':
            if d.apply_mode!='NEXT_REQUEST':return False
            comparison=replace(d,apply_mode='INITIALIZE_ONLY')
        if not matches(comparison,r) or d.validator!=((r.validator,) if r.validator else ()) or not set(r.dependencies)<=set(d.dependencies):return False
...
def _domain_definitions(kind,identity,definitions) -> bool:
    ...
    for r in requirements:
        d=by_key[r.key]
        if (not (matches_content_definition(d,r,sensitivity='administrator' if d.key in PROTECTED_KEYS else 'public') if kind=='content' else matches_runtime_definition(d,r,kind=='platform'))
...
def _runtime_capacity(candidate) -> bool:
    """Check configurable windows against the implemented bounded material path."""
    runtime = candidate.runtime
    for platform in candidate.platforms:
        history, target, recent = (platform.count(key) for key in
            ('history_context_count', 'target_count', 'recent_context_count'))
        count = history + target + recent
        if (count > cast(int, candidate.text.record('cognition.text_context')['member_limit'])
                or target > cast(int, candidate.text.record('cognition.text_context')['target_limit'])
                or platform.count('normal_soft_limit') < target + recent):
            return False
        if bool(platform.parameter('idle_tail_enabled')) != (platform.count('idle_timeout_ms') > 0):
            return False
        # Complete event and interpretation carriers plus fixed context overhead.
        bound = count * (runtime.integer('ingress.event_max_bytes') +
            candidate.content.integer('media.event_occurrence_limit') *
            candidate.content.integer('media.interpretation_record_max_bytes')) + 32768
        if bound > min(runtime.integer('learning.material_max_bytes'), runtime.integer('learning.input_units_limit')):
            return False
    return True


def resolve_managed_configuration(foundation:object,runtime:object,platforms:object,content:object,
        information:object,text:object,protected_directories:object,material_contracts:object) -> ManagedConfigurationOk[ManagedConfigurationCandidate] | ManagedConfigurationErr:
    ...
    if type(contract) is not ManagedMaterialContract or contract!=bind_managed_material(identity):return fail('DEFINITION_MISMATCH','material','METADATA_MISMATCH')
    ...
    candidate=object.__new__(ManagedConfigurationCandidate)
    ...
    from .managed_codec import ConfigurationCapacityExceeded
    try:
    ...
    return ManagedConfigurationOk(candidate)


def managed_snapshot_issue(candidate:object) -> ManagedConfigurationError | None:
    """Revalidate native ownership, metadata, values and all format relationships."""
    failure=configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','managed_snapshot_issue').error
    if type(candidate) is not ManagedConfigurationCandidate:return failure
    try:
        ...
        result=resolve_managed_configuration(*candidate_inputs(candidate,directories))
        if type(result) is not ManagedConfigurationOk:return failure
        from .managed_codec import candidate_values
