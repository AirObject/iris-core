...
from companion_memory.persistence import (
    ...
    PersistenceService,PersistenceError,RecoveryHandle,Unconfirmed,RecordSchema,ResultBoundCommand,ResultBoundCommandDefinition,
...
from .material_contracts import MaterialContract
from .persistent_codec import encode_entry,decode_entry
from .runtime_resolution import (
    ConfigurationCandidate,RuntimeConfigurationErr,RuntimeConfigurationOk,
    configuration_failure,resolve_runtime_configuration,runtime_snapshot_issue,
)
from .snapshots import PresentValue
from .persistent_results import ConfigurationCommitted,ConfigurationNotCommitted,ConfigurationRejected,ConfigurationUnconfirmed

ID=ScalarSchema('identifier')
INT=ScalarSchema('integer')
DOMAINS=SequenceSchema(RecordSchema((Field('domain_id',ID),Field('revision',INT))),1,66)
...
def _digest(entries: tuple[tuple[str,str],...]) -> str:
    return hashlib.sha256(_dump(entries).encode()).hexdigest()


def _platform_domain_id(platform_id:str) -> str:
    # Preserve the complete external binding in the catalog; audit IDs stay bounded.
    return 'platform:'+hashlib.sha256(platform_id.encode('utf-8')).hexdigest()


def _candidate_values(candidate: ConfigurationCandidate) -> dict[str,object]:
    if runtime_snapshot_issue(candidate) is not None:
        raise ValueError('A valid native configuration candidate is required.')
    domains=[]
    for identity,view in [('foundation',candidate.foundation),('runtime',candidate.runtime)]+[(_platform_domain_id(p.platform_id),p) for p in candidate.platforms]:
        entries=tuple((entry.definition.key,encode_entry(entry)) for entry in view.list_entries())
        if not 1<=len(entries)<=1024:
            raise ValueError('Configuration domain has unsupported size.')
        domains.append({'domain_id':identity,'digest':_digest(entries),'entries':[{'parameter_key':key,'body':body} for key,body in entries]})
    catalog=_dump({'version':1,'domains':[{'domain_id':d['domain_id'],'revision':1} for d in domains],
                   'materials':[{'platform_id':m.platform_id,'participant':m.participant,'event_format':m.event_format,'material_format':m.material_format,'template':m.template,'bound_rule':m.bound_rule,'version':m.version} for m in candidate.material_contracts]})
...
class StoredRuntimeConfiguration:
...
class ConfigurationAssembly:
    ...
    def __init__(self):
        self.repository=create_configuration_repository()
        self._binding: ConfigurationBinding | None = None
        requirement=AuditRequirement('configuration','configuration_initialized','CONFIGURATION_INITIALIZED',1,('INITIALIZE',),CHANGE)
        definition=ResultBoundCommandDefinition('configuration','initialize',1,
            RecordSchema((Field('catalog',BoundedTextSchema(8192)),Field('domains',SequenceSchema(RecordSchema((Field('domain_id',ID),Field('digest',ID),Field('entries',SequenceSchema(RecordSchema((Field('parameter_key',BoundedTextSchema(8192)),Field('body',BoundedTextSchema(8192)))),1,1024)))),1,66)))),
    ...
    def bind(self,storage:PersistenceService,instance_id:str) -> ConfigurationBinding:
        if self._binding is not None:
            raise ValueError('Configuration assembly is already bound.')
        binding=ConfigurationBinding(self,storage,instance_id)
...
class ConfigurationBinding:
    ...
    def __init__(self,assembly:ConfigurationAssembly,storage:PersistenceService,instance_id:str):
    ...
    async def persist_initial_configuration(self,operation_key:object,candidate:object,*,actor:str,protected_directories:object) -> object:
        """Bound the complete publication/load while retaining an unfinished owner."""
        operation='persist_initial_configuration'
        if self._closed or runtime_snapshot_issue(candidate) is not None:return configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH',operation)
        assert type(candidate) is ConfigurationCandidate
    ...
    async def _publish(self,operation_key,command,handle,candidate,protected_directories):
        ...
        if type(loaded) is RuntimeConfigurationErr:return ConfigurationCommitted(confirmed.receipt,confirmed.source,None,loaded.error,self._storage.get_health().cleanup_pending)
        assert type(loaded) is RuntimeConfigurationOk
    ...
    async def load_configuration_snapshot(self,snapshot_id:object,material_contracts:tuple[MaterialContract,...],protected_directories:object,*,bootstrap:ConfigurationCandidate):
        """Bounded whole-domain reconstruction; timeout cannot release a live reader."""
        if self._closed or runtime_snapshot_issue(bootstrap) is not None:return configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','load_configuration_snapshot')
        if self._tasks:return configuration_failure('PERSISTENCE_FAILED','storage','READ_FAILED','load_configuration_snapshot')
        ...
        return task.result() if done else configuration_failure('PERSISTENCE_FAILED','storage','READ_FAILED','load_configuration_snapshot')

    async def _load(self,snapshot_id:object,material_contracts:tuple[MaterialContract,...],protected_directories:object,*,bootstrap:ConfigurationCandidate) -> RuntimeConfigurationOk[StoredRuntimeConfiguration] | RuntimeConfigurationErr:
        """Reconstruct complete definitions and values, refusing missing or altered rows."""
        operation='load_configuration_snapshot'
        ...
        try:
            ...
            if set(catalog)!= {'version','domains','materials'} or type(catalog['version']) is not int or catalog['version']!=1:
            ...
            for item in catalog['domains']:
                ...
                if len(metadata)!=1 or len(keys)!=metadata[0]['item_count'] or len(keys)>1024 or identity in domains:
            ...
            resolved=resolve_runtime_configuration(domains['foundation'],domains['runtime'],platforms,protected_directories,material_contracts)
            if type(resolved) is not RuntimeConfigurationOk:
            ...
            stored=object.__new__(StoredRuntimeConfiguration)
            ...
            return RuntimeConfigurationOk(stored)
        except OSError:
...
def stored_configuration_issue(value:object) -> bool:
    ...
    if type(value) is not StoredRuntimeConfiguration:return True
    try:return object.__getattribute__(value,'_issuer') is not _STORED_ISSUER or runtime_snapshot_issue(value.candidate) is not None
