"""Independent complete semantic configuration resolution and native applicability.
...
from .semantic_validation import validate_semantic_values, validate_inherited_vector, validate_relationships
from .semantic_schema import SEMANTIC_PROVIDER_REQUIREMENTS, semantic_definitions, matches_semantic_definition
...
class SemanticConfigurationError:
    code: str
    operation: str
    field: str
    reason: str
...
@dataclass(frozen=True,slots=True)
class SemanticConfigurationErr:
    error: SemanticConfigurationError


@dataclass(frozen=True,slots=True)
class SemanticConfigurationOk[T]:
    value: T


def configuration_failure(code: str, field: str, reason: str, operation: str='resolve_semantic_configuration') -> SemanticConfigurationErr:
    return SemanticConfigurationErr(SemanticConfigurationError(code,operation,field,reason))


from .text_resolution import TextMaterialContract, bind_text_material


@dataclass(frozen=True,slots=True,init=False)
class SemanticSettingsSnapshot(ContentSettingsSnapshot):
...
@dataclass(frozen=True,slots=True,init=False)
class SemanticConfigurationCandidate:
    foundation: EffectiveSnapshot
    runtime: ContentSettingsSnapshot
    content: ContentSettingsSnapshot
    information: SemanticSettingsSnapshot
    text: SemanticSettingsSnapshot
...
def _foundation_definitions(definitions) -> bool:
    ...
    bindings={r.validator:(r.key,r.type) for r in SEMANTIC_PROVIDER_REQUIREMENTS if r.validator}
...
def _domain_definitions(kind,identity,definitions) -> bool:
    ...
    if kind=='text':return tuple(d.key for d in definitions)==tuple(sorted(d['key'] for d in (*text_definitions(),*semantic_definitions()))) and all(matches_text_definition(d) or matches_semantic_definition(d) for d in definitions)
...
def _foundation_values(snapshot,protected_directories):
    ...
    for e in entries:
        ...
        for v in e.definition.validator:
            if v.startswith(('text_','semantic_')):continue
...
def resolve_semantic_configuration(foundation:object,runtime:object,platforms:object,content:object,
        information:object,text:object,protected_directories:object,material_contracts:object) -> SemanticConfigurationOk[SemanticConfigurationCandidate] | SemanticConfigurationErr:
    ...
    for kind,identity,registry,explicit,definitions in prepared:
        ...
        if kind=='foundation':view=EffectiveSnapshot._from_entries(registry,r.value)
            ...
            cls=ContentPlatformSnapshot if kind=='platform' else SemanticSettingsSnapshot if kind in ('text','information') else ContentSettingsSnapshot
    ...
    if len(all_states)!=124 or any(type(all_states.get(dep)) is not PresentValue for view in built.values() for e in view.list_entries() for dep in e.definition.dependencies):
    ...
    candidate=object.__new__(SemanticConfigurationCandidate)
    ...
    try:
        ...
        validate_semantic_values(candidate.foundation,candidate.text)
        ...
        from .semantic_codec import candidate_values
    ...
    return SemanticConfigurationOk(candidate)


def semantic_snapshot_issue(candidate:object) -> SemanticConfigurationError | None:
    """Revalidate native ownership, metadata, values and all format relationships."""
    failure=configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','semantic_snapshot_issue').error
    if type(candidate) is not SemanticConfigurationCandidate:return failure
    try:
        ...
        result=resolve_semantic_configuration(*candidate_inputs(candidate,directories))
        if type(result) is not SemanticConfigurationOk:return failure
        from .semantic_codec import candidate_values
