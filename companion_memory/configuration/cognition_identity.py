...
from .managed_persistence import StoredManagedConfiguration, stored_managed_configuration_issue

type StoredCognitionConfiguration = StoredDailyConfiguration | StoredDreamConfiguration | StoredManagedConfiguration
...
def stored_cognition_configuration_issue(value: object, *, storage: PersistenceService | None = None) -> str | None:
    ...
    if type(value) is StoredManagedConfiguration:
        return stored_managed_configuration_issue(value)
...
from .managed_persistence import ManagedConfigurationAssembly, ManagedConfigurationBinding
...
type CognitionCandidate = DailyConfigurationCandidate | DreamConfigurationCandidate | ManagedConfigurationCandidate
type CognitionPublisher = DailyConfigurationBinding | DreamConfigurationBinding | ManagedConfigurationBinding
...
def cognition_candidate_values(candidate: CognitionCandidate):
    ...
    if type(candidate) is ManagedConfigurationCandidate:
...
def cognition_candidate_issue(candidate: object):
    if type(candidate) is ManagedConfigurationCandidate:
        return managed_snapshot_issue(candidate)
    if type(candidate) is DreamConfigurationCandidate:
        return dream_snapshot_issue(candidate)
    return daily_snapshot_issue(candidate)


def bind_cognition_configuration(assembly: DailyConfigurationAssembly | DreamConfigurationAssembly | ManagedConfigurationAssembly,
                                 storage: PersistenceService, instance: str, candidate: CognitionCandidate):
    if type(assembly) is ManagedConfigurationAssembly and type(candidate) is ManagedConfigurationCandidate:
...
async def persist_cognition_configuration(publisher: CognitionPublisher, key: str, candidate: CognitionCandidate,
    ...
    if type(publisher) is ManagedConfigurationBinding and type(candidate) is ManagedConfigurationCandidate:
        return await publisher.persist_managed_configuration(key,candidate,actor=actor,protected_directories=protected_directories)
...
async def initialize_cognition_roots(publisher: CognitionPublisher,key: str,stored: StoredCognitionConfiguration):
    if type(publisher) is ManagedConfigurationBinding and type(stored) is StoredManagedConfiguration:
