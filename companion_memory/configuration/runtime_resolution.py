"""Atomic full-domain runtime configuration resolution and immutable bound views.
...
from .material_contracts import MaterialContract, valid_material
...
from .runtime_schema import RUNTIME_REQUIREMENTS, matches_runtime_definition, platform_requirements
...
@dataclass(frozen=True, slots=True)
class RuntimeConfigurationErr:
    error: RuntimeConfigurationError


@dataclass(frozen=True, slots=True)
class RuntimeConfigurationOk[T]:
    value: T


def configuration_failure(code: str, field: str, reason: str, operation: str = 'resolve_runtime_configuration') -> RuntimeConfigurationErr:
    return RuntimeConfigurationErr(RuntimeConfigurationError(code, operation, field, reason))
...
class RuntimeSettingsSnapshot:
    ...
    def get_entry(self, key: object) -> ResolutionOk[SnapshotEntry] | RuntimeConfigurationErr:
        if type(key) is not str:
            return configuration_failure('INVALID_INPUT','identity','INVALID_SHAPE','runtime_snapshot_issue')
        for entry in self._entries:
            if entry.definition.key == key:
                return ResolutionOk(entry)
        return configuration_failure('INVALID_INPUT','identity','UNKNOWN_KEY','runtime_snapshot_issue')
...
@dataclass(frozen=True, slots=True, init=False, eq=False)
class PlatformSettingsSnapshot(RuntimeSettingsSnapshot):
...
class ConfigurationCandidate:
    ...
    platforms: tuple[PlatformSettingsSnapshot, ...]
    ...
    def platform(self, platform_id: str) -> PlatformSettingsSnapshot:
...
def resolve_runtime_configuration(foundation: object, runtime: object, platforms: object, protected_directories: object, material_contracts: object) -> RuntimeConfigurationOk[ConfigurationCandidate] | RuntimeConfigurationErr:
    """Validate all domains, then publish exactly one isolated candidate.

    Five explicit carriers are mandatory. Full structural and capability checks
    ...
    first, second = _domain(foundation), _domain(runtime)
    if first is None or second is None or (type(platforms) is not list and type(platforms) is not tuple) or not 1 <= len(platforms) <= 64 or (type(material_contracts) is not list and type(material_contracts) is not tuple):
        return fail('INVALID_INPUT','context','INVALID_SHAPE')
    domains = [('foundation','',first),('runtime','',second)]
    ...
    for kind, identity, _, _, definitions in prepared:
        if kind == 'foundation':
            ...
            continue
        requirements = platform_requirements(identity) if kind == 'platforms' else RUNTIME_REQUIREMENTS
        ...
        for d in definitions:
            ...
            if any(v not in bindings or bindings[v] != d.key for v in d.validator):
    ...
    for kind, identity, _, _, definitions in prepared:
        ...
        for r in sorted(requirements,key=lambda r:r.key):
            ...
            if not matches_runtime_definition(d,r,kind == 'platforms'):
                return fail('DEFINITION_MISMATCH',kind,'METADATA_MISMATCH')
    ...
    if 'logging.event_max_bytes' not in foundation_values or not any(foundation_values.get(k) is True for k in ('logging.console_enabled','logging.file_enabled')):
        return fail('UNSUPPORTED_CAPABILITY','foundation','FOUNDATION_UNSUPPORTED')
    built: dict[tuple[str,str],RuntimeSettingsSnapshot] = {}
    for kind,identity,registry,explicit,definitions in prepared:
        if kind == 'foundation':
            continue
        result = _resolve_entries(definitions,explicit)
        if type(result) is ResolutionErr:
            return fail('VALUE_INVALID',kind,'RANGE_INVALID')
        assert type(result) is ResolutionOk
        view = PlatformSettingsSnapshot._issue(registry,result.value) if kind == 'platforms' else RuntimeSettingsSnapshot._issue(registry,result.value)
        if kind == 'platforms':
    ...
    ps = tuple(cast(PlatformSettingsSnapshot,built['platforms',identity]) for identity in sorted(ids))
    if any(not valid_material(m) for m in contracts):return fail('VALUE_INVALID','platforms','BUDGET_INVALID')
    issue = _relationships(resolved.value,settings,ps,tuple(contracts))
    if issue is not None:
        return issue
    candidate = object.__new__(ConfigurationCandidate)
    object.__setattr__(candidate,"_issuer",_CANDIDATE_ISSUER)
    for key,value in dict(foundation=resolved.value,runtime=settings,platforms=ps,material_contracts=tuple(sorted(contracts,key=lambda m:m.platform_id))).items():
        object.__setattr__(candidate,key,value)
    return RuntimeConfigurationOk(candidate)


def _relationships(foundation: EffectiveSnapshot, runtime: RuntimeSettingsSnapshot, platforms: tuple[PlatformSettingsSnapshot,...], contracts: tuple[MaterialContract,...]) -> RuntimeConfigurationErr | None:
    ...
    capacity = 6*max(n('ingress.event_max_bytes'),n('learning.material_max_bytes'),8192)+8192
    ...
    for platform in platforms:
        ...
        if h+t+r > 128 or platform.count('normal_soft_limit') < t+r:
        ...
        budget = contract.window_bound(h+t+r,n('ingress.event_max_bytes'))
        ...
        if 146+1040+132+119+budget+(10+h+t+r) > cast(int,f['provider.request_max_bytes']):
            return fail('VALUE_INVALID','platforms','CAPACITY_INSUFFICIENT')
    return None


def runtime_snapshot_issue(candidate: object) -> RuntimeConfigurationError | None:
    ...
    failure=configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','runtime_snapshot_issue').error
    if type(candidate) is not ConfigurationCandidate:return failure
    try:
        if object.__getattribute__(candidate,'_issuer') is not _CANDIDATE_ISSUER:return failure
        if (type(candidate.foundation) is not EffectiveSnapshot or type(candidate.runtime) is not RuntimeSettingsSnapshot
                or type(candidate.platforms) is not tuple or not 1<=len(candidate.platforms)<=64
                or type(candidate.material_contracts) is not tuple or any(type(p) is not PlatformSettingsSnapshot for p in candidate.platforms)
        ...
        for view,requirements in ((candidate.runtime,RUNTIME_REQUIREMENTS),*((p,platform_requirements(p.platform_id)) for p in candidate.platforms)):
            definitions=tuple(e.definition for e in view.list_entries())
            by_key={d.key:d for d in definitions}
            for required in requirements:
                if required.key not in by_key or not matches_runtime_definition(by_key[required.key],required,type(view) is PlatformSettingsSnapshot):return failure
        ...
        issue=_relationships(candidate.foundation,candidate.runtime,candidate.platforms,candidate.material_contracts)
