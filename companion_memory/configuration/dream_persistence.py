...
from .dream_resolution import DreamMaterialContract as MaterialContract
...
from .dream_codec import candidate_values as _candidate_values
from .dream_resolution import (
    DreamConfigurationCandidate,DreamConfigurationErr,DreamConfigurationOk,
    configuration_failure,resolve_dream_configuration,dream_snapshot_issue,
)
from .dream_persistent_results import ConfigurationCommitted,ConfigurationNotCommitted,ConfigurationRejected,ConfigurationUnconfirmed
...
def stored_dream_configuration_issue(value: object) -> str | None:
    """Check native durable issuance and the complete reconstructed candidate."""
    if type(value) is not StoredDreamConfiguration or getattr(value, '_issuer', None) is not _STORED_ISSUER:
    ...
    try:
        ...
        return 'DEFINITION_MISMATCH' if dream_snapshot_issue(value.candidate) is not None else None
...
class DreamConfigurationAssembly:
    ...
    def __init__(self,memory_catalog:StatementCatalog|None=None,retrieval_catalog:StatementCatalog|None=None):
        ...
        self.repository=replace(repository, definition=replace(repository.definition, schema_version=7))
        self._binding: DreamConfigurationBinding | None = None
        requirement=AuditRequirement('configuration','configuration_initialized','CONFIGURATION_INITIALIZED',1,('INITIALIZE',),CHANGE)
        definition=ResultBoundCommandDefinition('configuration','initialize_dream_configuration',1,
        ...
        from companion_memory.persistence.command_capacity import _issue_dream_configuration_capacity
        definition=replace(definition,capacity_policy=_issue_dream_configuration_capacity(self))
    ...
    def bind(self,storage:PersistenceService,instance_id:str,bootstrap:DreamConfigurationCandidate) -> DreamConfigurationBinding:
        if self._binding is not None:
            raise ValueError('Configuration assembly is already bound.')
        if dream_snapshot_issue(bootstrap) is not None:
            raise ValueError('A complete native daily configuration is required.')
        binding=DreamConfigurationBinding(self,storage,instance_id,bootstrap)
        self._binding=binding
        return binding


type ConfigurationWriteResult = ConfigurationCommitted | ConfigurationNotCommitted | ConfigurationRejected | ConfigurationUnconfirmed | DreamConfigurationErr
type ConfigurationReadResult = DreamConfigurationOk[StoredDreamConfiguration] | DreamConfigurationErr


class DreamConfigurationBinding:
    """Restricted initialization and point loads through configuration-owned ports."""
    def __init__(self,assembly:DreamConfigurationAssembly,storage:PersistenceService,instance_id:str,bootstrap:DreamConfigurationCandidate):
    ...
    async def initialize_business_roots(self,key:str,stored:StoredDreamConfiguration):
        ...
        """
        if self._closed or self._root_operation is None or stored_dream_configuration_issue(stored) is not None or self._tasks:
            raise InvalidValue()
    ...
    def participate_snapshot(self, uow: UnitOfWork, expected: StoredDreamConfiguration) -> bool:
        """Verify the active durable snapshot inside another owner's transaction."""
        if (self._closed or stored_dream_configuration_issue(expected) is not None
    ...
    async def persist_dream_configuration(self,operation_key:object,candidate:object,*,actor:str,protected_directories:object) -> ConfigurationWriteResult:
        """Bound the complete publication/load while retaining an unfinished owner."""
        operation='persist_dream_configuration'
        if self._closed or type(candidate) is not DreamConfigurationCandidate or dream_snapshot_issue(candidate) is not None:return configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH',operation)
        from .dream_codec import candidate_inputs
        preflight = resolve_dream_configuration(*candidate_inputs(candidate, protected_directories))
        if type(preflight) is DreamConfigurationErr:
            return DreamConfigurationErr(replace(preflight.error, operation=operation))
    ...
    async def _publish(self, operation_key: object, command: ResultBoundCommand, handle: RecoveryHandle,
                       candidate: DreamConfigurationCandidate, protected_directories: object) -> ConfigurationWriteResult:
        with CompletionScope() as completion:
            ...
            if type(result) is not MappingProxyType:
                failure = configuration_failure('INTEGRITY_FAILURE', 'storage', 'CONTENT_MISMATCH', 'persist_dream_configuration')
            ...
            if isinstance(loaded, DreamConfigurationOk):
    ...
    async def active_snapshot_id(self) -> object:
        ...
        if self._closed:
            return configuration_failure('ACCESS_DENIED', 'identity', 'BINDING_MISMATCH', 'load_dream_configuration')
    ...
    async def _read_active_snapshot_id(self) -> object:
        ...
        if type(result) is Found and type(result.value) is tuple:
            ...
            if type(row) is not MappingProxyType:
                return configuration_failure('INTEGRITY_FAILURE', 'storage', 'CONTENT_MISMATCH', 'load_dream_configuration')
            return Found(row['snapshot_id'])
        return result

    async def load_dream_configuration(self,snapshot_id:object,material_contracts:tuple[MaterialContract,...],protected_directories:object,*,bootstrap:DreamConfigurationCandidate) -> ConfigurationReadResult:
        """Bounded whole-domain reconstruction; timeout cannot release a live reader."""
        if self._closed or dream_snapshot_issue(bootstrap) is not None:return configuration_failure('ACCESS_DENIED','identity','BINDING_MISMATCH','load_dream_configuration')
        if self._tasks:return configuration_failure('PERSISTENCE_FAILED','storage','READ_FAILED','load_dream_configuration')
        ...
        return logical.result() if done else configuration_failure('PERSISTENCE_FAILED','storage','READ_FAILED','load_dream_configuration')

    async def _load(self,snapshot_id:object,material_contracts:tuple[MaterialContract,...],protected_directories:object,*,bootstrap:DreamConfigurationCandidate) -> DreamConfigurationOk[StoredDreamConfiguration] | DreamConfigurationErr:
        """Reconstruct complete definitions and values, refusing missing or altered rows."""
        operation='load_dream_configuration'
        ...
        try:
            ...
            if set(catalog)!= {'version','domains','materials'} or type(catalog['version']) is not int or catalog['version']!=7:
            ...
            from .dream_resolution import freeze_daily_domains
            ...
            resolved=resolve_dream_configuration(domains['foundation'],domains['runtime'],platforms,domains['content'],domains['information'],domains['daily_cognition'],protected_directories,material_contracts)
            ...
            stored=object.__new__(StoredDreamConfiguration)
            ...
            return DreamConfigurationOk(stored)
        except OSError:
...
def stored_configuration_issue(value:object) -> bool:
    ...
    if type(value) is not StoredDreamConfiguration:return True
    try:return object.__getattribute__(value,'_issuer') is not _STORED_ISSUER or dream_snapshot_issue(value.candidate) is not None
