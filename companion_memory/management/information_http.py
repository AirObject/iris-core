...
class InformationHTTP:
    ...
    def __init__(self, configuration: StoredInformationConfiguration | StoredTextConfiguration | StoredSemanticConfiguration | StoredCognitionConfiguration, gate: ContentGate):
        if type(configuration) not in (StoredInformationConfiguration,StoredTextConfiguration) and (stored_cognition_configuration_issue(configuration) if (type(configuration) is StoredDailyConfiguration or type(configuration) is StoredDreamConfiguration or type(configuration) is StoredManagedConfiguration) else stored_semantic_configuration_issue(configuration)) is not None:
