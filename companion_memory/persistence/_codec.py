...
def command_descriptor(definition: CommandSpec) -> Value:
    ...
    if capacity is not None:
        result['frozen_carrier_policy'] = MappingProxyType({'kind': 'DAILY_CONFIGURATION_INITIALIZATION' if definition.operation_kind=='initialize_daily_configuration' else 'SEMANTIC_CONFIGURATION_INITIALIZATION' if definition.operation_kind=='initialize_semantic' else 'TEXT_CONFIGURATION_INITIALIZATION', 'max_bytes': capacity})
...
type AssemblyFormat = Literal['LEGACY', 'LOCAL_INFORMATION_V1', 'MODEL_TEXT_LEARNING_V1', 'ASYNC_SEMANTIC_V1', 'DAILY_COGNITION_V1']
...
def assembly_value(repositories: tuple[RepositoryDefinition, ...], commands: tuple[CommandSpec, ...],
    ...
    """
    if type(assembly_format) is not str or assembly_format not in ('LEGACY', 'LOCAL_INFORMATION_V1', 'MODEL_TEXT_LEARNING_V1', 'ASYNC_SEMANTIC_V1', 'DAILY_COGNITION_V1'):
        raise InvalidValue()
    from .command_capacity import declared_capacity
    ...
    if assembly_format not in ('ASYNC_SEMANTIC_V1','DAILY_COGNITION_V1') and any(semantic_declared(command) for command in commands):
        raise InvalidValue()
    if assembly_format not in ('MODEL_TEXT_LEARNING_V1','ASYNC_SEMANTIC_V1', 'DAILY_COGNITION_V1') and any(declared(command) for command in commands):
        raise InvalidValue()
    if policies != (1 if assembly_format in ('MODEL_TEXT_LEARNING_V1','ASYNC_SEMANTIC_V1', 'DAILY_COGNITION_V1') else 0):
        raise InvalidValue()
    expected_initialization={'MODEL_TEXT_LEARNING_V1':'initialize_text_learning',
        'ASYNC_SEMANTIC_V1':'initialize_semantic','DAILY_COGNITION_V1':'initialize_daily_configuration'}.get(assembly_format)
    ...
    if assembly_format in ('ASYNC_SEMANTIC_V1','DAILY_COGNITION_V1'):
        for repository in repositories:
            for table in repository.tables:
                if table.record_schemas:encode_value(table_descriptor(table),8388608 if assembly_format=='DAILY_COGNITION_V1' else 32768)
        return encode_value(MappingProxyType(contents),8388608 if assembly_format=='DAILY_COGNITION_V1' else 4194304)
...
def valid_assembly_encoding(data: object, assembly_format: AssemblyFormat) -> bool:
    ...
    if type(data) is not bytes:
        return False
    limit = 8388608 if assembly_format=='DAILY_COGNITION_V1' else 4194304 if assembly_format=='ASYNC_SEMANTIC_V1' else 3145728 if assembly_format in ('LOCAL_INFORMATION_V1', 'MODEL_TEXT_LEARNING_V1') else 1048576
    try:
        value = decode_value(data, limit)
        keys = {'repositories', 'commands'}
        if assembly_format in ('LOCAL_INFORMATION_V1', 'MODEL_TEXT_LEARNING_V1','ASYNC_SEMANTIC_V1', 'DAILY_COGNITION_V1'):
            keys.add('static_format')
        if type(value) is not dict or set(value) != keys:
            return False
        if assembly_format in ('LOCAL_INFORMATION_V1', 'MODEL_TEXT_LEARNING_V1','ASYNC_SEMANTIC_V1', 'DAILY_COGNITION_V1') and value['static_format'] != assembly_format:
            return False
