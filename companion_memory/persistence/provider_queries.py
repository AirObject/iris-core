...
def aggregate_statement(*, text_generation: bool = False, embedding: bool = False) -> StatementDefinition:
    ...
    for state in attempt_states:
        ...
        fields.append(name)
    usage_fields = ('input_tokens','total_tokens','input_items','embedding_dimensions','cache_read_tokens') if embedding else ((*USAGE_FIELDS, 'total_tokens') if text_generation else USAGE_FIELDS)
    checks = [f"json_type(a.body,'$.usage.{name}')='integer' AND json_extract(a.body,'$.usage.{name}')>=0"
              for name in (("known_subtotal_atoms", "held_atoms") if text_generation or embedding else ("known_subtotal_atoms", "held_atoms", "estimated_cost_atoms"))]
    if text_generation or embedding:
